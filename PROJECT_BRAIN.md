...
- **Removed auto-budget pre-fill**: carrier bid form no longer auto-fills with shipper's budget amount. Carriers must enter their own bid manually. Budget hint text still visible below the input for reference
- **Platform fee disclosure on post-load form**: Budget step (step 3) now shows a notice: "A 7% platform fee will be added to the carrier's accepted bid amount" with a live example when budget is entered
- **Improved bid acceptance fee breakdown**: replaced single-line "Total cost: ₦X (incl. 7% fee)" with an itemized breakdown (Carrier fee / Platform fee / You pay) in the confirmation dialog
- Modified: carrier loads/[id]/page.tsx, shipper post-load/page.tsx, shipper loads/[id]/page.tsx
- Build passes: 0 errors, pushed to GitHub
